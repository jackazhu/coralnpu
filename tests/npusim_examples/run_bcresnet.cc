...
int main(int argc, char** argv) {
  ...
  if (model->version() != TFLITE_SCHEMA_VERSION) {
    std::strncpy(inference_status_message, "schema mismatch",
                 sizeof(inference_status_message));
    return -1;
  }

  BcResnetOpResolver op_resolver;
  if (RegisterOps(op_resolver) != kTfLiteOk) {
    std::strncpy(inference_status_message, "op resolver failed",
                 sizeof(inference_status_message));
    return -1;
  }
  ...
  if (interpreter.AllocateTensors() != kTfLiteOk) {
    std::strncpy(inference_status_message, "AllocateTensors failed",
                 sizeof(inference_status_message));
    return -1;
  }
  ...
  if (input == nullptr) {
    std::strncpy(inference_status_message, "input tensor null",
                 sizeof(inference_status_message));
    return -1;
  }
  ...
  if (interpreter.Invoke() != kTfLiteOk) {
    std::strncpy(inference_status_message, "Invoke failed",
                 sizeof(inference_status_message));
    return -1;
  }
  ...
  if (output == nullptr) {
    std::strncpy(inference_status_message, "output tensor null",
                 sizeof(inference_status_message));
    return -1;
  }
  std::memcpy(inference_output, output->data.data, sizeof(inference_output));
  std::strncpy(inference_status_message, "Invoke successful",
               sizeof(inference_status_message));
  inference_status = 0;
  return 0;
}
