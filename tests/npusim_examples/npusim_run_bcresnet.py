...
def run_bcresnet():
    ...
    # BCResNet exported input shape: [1, 1, 40, 101], float32.
    if symbol_map.get("inference_input"):
        input_data = np.random.randn(1 * 1 * 40 * 101).astype(np.float32)
    ...
    if symbol_map.get("inference_output"):
        output_raw = npu_sim.read_memory(symbol_map["inference_output"], 35 * 4)
        output = np.frombuffer(output_raw, dtype=np.float32)
        max_idx = int(np.argmax(output))
        print(
            f"Output info: Top index {max_idx} with value {output[max_idx]} "
            f"from first 8 logits {output[:8]}"
        )
