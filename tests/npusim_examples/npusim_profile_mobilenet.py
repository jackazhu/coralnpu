...
"""Per-layer profiling runner for MobileNet using CORALNPU_OP_PROFILE=1 binary."""
...
def run_profile_mobilenet():
    print("Running MobileNet profiling build...")
    npu_sim = CoralNPUV2Simulator(highmem_ld=True, exit_on_ebreak=True)
    r = runfiles.Create()
    elf_file = r.Rlocation(
        'coralnpu_hw/tests/npusim_examples/run_full_mobilenet_v1_profile_binary.elf')
    ...
    print(f"PERF_CYCLES|runner=npusim|test=npusim_profile_mobilenet|cycles={cycle_count}")
